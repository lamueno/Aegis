...
me.logmethodcall = function(module, calltype)
	...
	if time > 100 or time < 0 or math.abs(time - manualtime) > 5 then
		time = 0
	elseif time > 10 then
		if mod.output.checktrace("info", me, "timing") then
			mod.output.printtrace(string.format("Time from %s %s is %s ms (manual says %s ms) (memory was %s).", module, calltype, me.formatdecimal(time), manualtime, memory))
		end
	end
...
me.printalldata = function(datatype, units)
	...
	mod.output.print(string.format("|cff6666ffThis is a listing of |cffffff00%s |cff6666ffusage. Units are |cffffff00%s.|r", datatype, units))
...
me.printdataset = function(name, set)
	...
	mod.output.print(string.format("%s:|r Total = %s, Avg = %s, Burst = %s, Recent = %s.", 
