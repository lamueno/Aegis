...
me.onevent = function()
  ...
  local sequence = {
		me.trigger.combat_state,
		-- me.trigger.combat_log,
...
me.trigger.combat_state = function()

  if event == "PLAYER_REGEN_DISABLED" then
    mod.combat.state.combat_begin = GetTime()
    mod.combat.state.combat_end = nil
    return true

  elseif event == "PLAYER_REGEN_ENABLED" then
    mod.combat.state.combat_end = GetTime()
    return true
  end
...
-- me.trigger.combat_log = function ()
-- 	if event == "UNIT_COMBAT" then
-- 		if arg1 == "player" then
-- 			me.prediction.playerexhaust = mod.my.health:exhaust()
-- 		elseif arg1 == "target" then
-- 			me.prediction.targetexhaust = mod.target.health:exhaust()
-- 		end
-- 	end
-- end
...
me.prediction.battleresult = function()

	local now = GetTime()
	local p = mod.my.health:exhaust() or now*2
	local t = mod.target.health:exhaust() or now*2

	if p - t < -10 then -- Player will die before target
		return "lost", p-now
	elseif p - t > 10 then  -- Target will die before player
		return "win", t-now
	else
		return "tie", nil
	end
...
me.prediction.text = function()
	...
	local function seconds_to_minutes(seconds)
		...
		elseif seconds < 3600 then
			...
			local sec = math.mod(seconds, 60)
			return format("%dm%ds", min, sec)
		elseif seconds >= 3600 then
			local hour = math.floor(seconds / 3600)
			local min = math.floor((seconds-3600*hour), 60)
			local sec = math.mod(seconds, 60)
			return format("%dh%dm%ds", hour, min, sec)
		end
	end

	result, battle_end_in = me.prediction.battleresult()
	if UnitAffectingCombat("player") then
		if result == "win" then
			return format("预计胜利 / 持续 %s", seconds_to_minutes(battle_end_in))
		elseif result == "lost" then
			return format("预计失败 / 持续 %s", seconds_to_minutes(battle_end_in))
...
me.conserve_rage = function(name)
  ...
  -- Imperical formula to conserve rage, esp for Shield Slam.
...
me.cast.Execute = function()
  ...
  if mod.libspell.SpellCanCast("execute") and mod.target.health.pct <= 20 and mod.libspell.SpellReadyIn("execute") == 0 then
...
me.cast.SunderArmor = function()
  ...
  if count then
    if count > 3 or me.state.lastsunder + 25 > GetTime() then
      return
    end
  end
...
me.cast.DebuffTreat = function ()
  if mod.my.buffed(mod.string.get("debuff", "Brood Affliction: Bronze")) then
		mod.libitem.use(mod.string.get("item", "Hourglass Sand"))
	end
